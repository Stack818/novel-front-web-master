            ...
            <form method="post" action="./register.html" id="form2">
              <div class="user_l">
                ...
                <ul class="log_list">
                  ...
                  <b>作品方向：</b>
                  <li>
                    <select v-model="book.workDirection" class="s_input" id="workDirection" name="workDirection"
                  ...
                  <li>
                    <select class="s_input" id="catId" name="catId" v-model="book.categoryId" @change="categoryChange">
                      <option :value="item.id" v-for="(item, index) in bookCategorys" :key="index">{{ item.name }}
                      </option>
                  ...
                  <li style="position: relative">
                    <el-upload class="avatar-uploader" :action="baseUrl + '/front/resource/image'" :show-file-list="false"
                      :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
                      <img :src="book.picUrl ? imgBaseUrl + book.picUrl : picUpload
                        " class="avatar" />
                    </el-upload>
                  </li>
                  <b>小说文件：</b>
                  <li style="position: relative">
                    <el-upload class="avatar-uploader" :action="baseUrl + '/front/resource/bookfile'"
                      :show-file-list="false" :on-success="handleFileSuccess" :before-upload="beforeFileUpload">
                    </el-upload>
                  ...
                  <li>
                    <textarea v-model="book.bookDesc" name="bookDesc" rows="5" cols="53" id="bookDesc"
                      class="textarea"></textarea>
                  </li>

                  <li>
                    <input type="button" @click="saveBook" name="btnRegister" value="提交" id="btnRegister"
                      class="btn_red" />
                  </li>
                </ul>
              </div>
...
export default {
  ...
  setup() {
    ...
    const beforeFileUpload = (rawFile) => {
      if (rawFile.type !== "bookFile/txt") {
        ElMessage.error("必须上传 txt 格式的文件!");
        return false;
      }
      return true;
    ...
    const handleFileSuccess = (response, uploadFile) => {
      state.book.bookFileUrl = response.data;
